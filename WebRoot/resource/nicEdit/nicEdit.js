...
var nicEditorConfig = bkClass.extend({
	buttons : {
		...
		'italic' : {name : __('变斜'), command : 'Italic', tags : ['EM','I'], css : {'font-style' : 'italic'}, key : 'i'},
	...
	iconsPath : 'resource/nicEdit/nicEditorIcons.gif',
	...
	iconList : {"xhtml":1,"bgcolor":2,"forecolor":3,"bold":4,"center":5,"hr":6,"indent":7,"italic":8,"justify":9,"left":10,"ol":11,"outdent":12,"right":13,"save":24,"strikethrough":15,"subscript":16,"superscript":17,"ul":18,"underline":19,"image":20,"link":21,"unlink":22,"close":23,"arrow":25}
...
var nicEditorPane = bkClass.extend({
	construct : function(elm,nicEditor,options,openButton) {
		...
		this.contain = new bkElement('div').setStyle({zIndex : '99', overflow : 'hidden', position : 'absolute', left : this.pos[0]+'px', top : this.pos[1]+'px'})
	...
	addForm : function(f,elm) {
		...
		for(itm in f) {
			...
			if(type == 'title') {
				...
				if(field.txt) {
					new bkElement('label').setAttributes({'for' : itm}).setContent(field.txt).setStyle({margin : '2px 4px', fontSize : '11px', width: '80px', lineHeight : '20px', textAlign : 'right', 'float' : 'left'}).appendTo(contain);
				}
				...
				switch(type) {
					...
					case 'content':
						...
						break;
					case 'addon':
						this.inputs[itm] = new bkElement('div').setStyle({border : '1px solid #ccc', 'float' : 'right'}).appendTo(contain);
						this.inputs[itm].innerHTML = val;
						break;
				}	
			}
		}
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({
	sel : {1 : '1&nbsp;(8px)', 2 : '2&nbsp;(10px)', 3 : '3&nbsp;(12px)', 4 : '4&nbsp;(14px)', 5 : '5&nbsp;(18px)'},
	init : function() {
		this.setDisplay('字号');
		for(itm in this.sel) {
...
var nicEditorFontFamilySelect = nicEditorSelect.extend({
	sel : {'宋体' : '宋体','黑体' : '黑体','Arial' : 'Arial','微软雅黑' : '微软雅黑', '魏碑' : '魏碑', 'Courier New' : 'Courier New', 'times new roman' : 'Times',  'verdana' : 'Verdana'},
	init : function() {
		this.setDisplay('字体');
		for(itm in this.sel) {
...
var nicLinkOptions = {
	buttons : {
		...
		'unlink' : {name : '去除链接',  command : 'unlink', noActive : true}
	}
};
/* END CONFIG */
...
var nicLinkButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		this.addForm({
			'' : {type : 'title', txt : '链接信息'},
			'href' : {type : 'text', txt : '地址', value : 'http://', style : {width: '180px'}},
			'title' : {type : 'text', txt : '标题', style : {width: '180px'}},
...
var nicImageButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		this.addForm({
			'' : {type : 'title', txt : "图片信息"},
			'src' : {type : 'text', txt : '地址', 'value' : 'http://', style : {width: '180px'}},
			'alt' : {type : 'text', txt : '提示', style : {width: '180px'}},
			'align' : {type : 'select', txt : '对齐', options : {none : '默认','left' : '左', 'right' : '右'}},
			'fromGalery' : {type : 'addon', value : "<a href='img.shtml?method=miniImageForm' class='boxy' title='选择相册图片'>选择相册图片</a>", style : {width: '180px'}}
		},this.im);
		$('.boxy').boxy();
	},
